...
client.on("ready", async () => {
  client.user.setPresence({ activity: { name: "Aref 💖 " }, status: "online" });
...
client.on("message", async message => {
  ...
  let embed = new MessageEmbed().setColor("#6d1d76").setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true, })).setFooter(`${client.users.cache.has(ayarlar.botOwner) ? client.users.cache.get(ayarlar.botOwner).tag : "Aref"}💖 Aref`).setTimestamp();
...
function cezalandir(kisiID, tur) {
  ...
  if (tur == "ban") return uye.ban({ reason: "Aref Guardian" }).catch();
};
...
client.on("guildMemberAdd", async member => {
  ...
  if (logKanali) { logKanali.send(new MessageEmbed().setColor("#4B0082").setTitle('Aref Guardian').setDescription(`${member},(\`\`${member.id}\`\`) botu, ${entry.executor},(\`\`${entry.executor.id}\`\`) tarafından sunucuya eklendi! Ekleyen kişi ve bot banlandı.`).setFooter(`${client.users.cache.has(ayarlar.botOwner) ? client.users.cache.get(ayarlar.botOwner).tag : "Aref"}💖 Aref`).setTimestamp()).catch(); } else { member.guild.owner.send(new MessageEmbed().setColor("#00ffdd").setTitle('Sunucuya Bot Eklendi!').setDescription(`${member} **(${member.id})** botu, ${entry.executor} **(${entry.executor.id})** tarafından sunucuya eklendi! Ekleyen kişi ve bot banlandı.`).setFooter(`${client.users.cache.has(ayarlar.botOwner) ? client.users.cache.get(ayarlar.botOwner).tag : "Aref"}💖 Aref`).setTimestamp()).catch(err => {}); };
});
...
client.on("guildMemberRemove", async member => {
  ...
  let logKanali = client.channels.cache.get(ayarlar.logChannelID);
  if (logKanali) { logKanali.send(new MessageEmbed().setColor("#4B0082").setTitle('Aref Guardian').setDescription(`${member},(\`\`${member.id}\`\`) üyesi, ${entry.executor},(\`\`${entry.executor.id}\`\`) tarafından sunucudan sağ tık ile kicklendi! Kickleyen kişi jaile atıldı.`).setFooter(`${client.users.cache.has(ayarlar.botOwner) ? client.users.cache.get(ayarlar.botOwner).tag : "Aref"}💖 Aref`).setTimestamp()).catch(); } else { member.guild.owner.send(new MessageEmbed().setColor("#00ffdd").setTitle('Sağ Tık Kick Atıldı!').setDescription(`${member} **(${member.id})** üyesi, ${entry.executor} (${entry.executor.id}) tarafından sunucudan sağ tık ile kicklendi! Kickleyen kişi jaile atıldı.`).setFooter(`${client.users.cache.has(ayarlar.botOwner) ? client.users.cache.get(ayarlar.botOwner).tag : "Aref"}💖 Aref`).setTimestamp()).catch(err => {}); };
});
...
client.on("guildBanAdd", async (guild, user) => {
  ...
  cezalandir(entry.executor.id, "jail");
  guild.members.unban(user.id, "Aref Guardian").catch(console.error);
  let logKanali = client.channels.cache.get(ayarlar.logChannelID);
  if (logKanali) { logKanali.send(new MessageEmbed().setColor("#4B0082").setTitle('Aref Guardian').setDescription(`${user},(\`\`${user.id}\`\`) üyesi, ${entry.executor},(\`\`${entry.executor.id}\`\`) tarafından sunucudan sağ tık ile banlandı! Banlayan kişi jaile atıldı. `).setFooter(`${client.users.cache.has(ayarlar.botOwner) ? client.users.cache.get(ayarlar.botOwner).tag : "Aref"}💖 Aref`).setTimestamp()).catch(); } else { guild.owner.send(new MessageEmbed().setColor("#00ffdd").setTitle('Sağ Tık Ban Atıldı!').setDescription(`${user} **(${user.id})** üyesi, ${entry.executor} **(${entry.executor.id})** tarafından sunucudan sağ tık ile banlandı! Banlayan kişi jaile atıldı.`).setFooter(`${client.users.cache.has(ayarlar.botOwner) ? client.users.cache.get(ayarlar.botOwner).tag : "Aref"}💖 Aref`).setTimestamp()).catch(err => {}); };
});
